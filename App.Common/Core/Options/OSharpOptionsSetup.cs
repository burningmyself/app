...
namespace Common.Core.Options
{
    ...
    public class OsharpOptionsSetup : IConfigureOptions<OsharpOptions>
    {
        ...
        public void Configure(OsharpOptions options)
        {
            ...
            //OAuth2
            section = _configuration.GetSection("Common:OAuth2");
            ...
            //MailSender
            section = _configuration.GetSection("Common:MailSender");
            MailSenderOptions sender = section.Get<MailSenderOptions>();
            if (sender != null)
            {
                if (sender.Password == null)
                {
                    sender.Password = _configuration["Common:MailSender:Password"];
                }
                options.MailSender = sender;
            }

            //JwtOptions
            section = _configuration.GetSection("Common:Jwt");
            JwtOptions jwt = section.Get<JwtOptions>();
            if (jwt != null)
            {
                if (jwt.Secret == null)
                {
                    jwt.Secret = _configuration["Common:Jwt:Secret"];
                }
                options.Jwt = jwt;
            }

            // RedisOptions
            section = _configuration.GetSection("Common:Redis");
            ...
            // SwaggerOptions
            section = _configuration.GetSection("Common:Swagger");
        ...
        /// <summary>
        /// 初始化上下文配置信息，首先以Common配置节点中的为准，
        /// 不存在Common节点，才使用ConnectionStrings的数据连接串来构造SqlServer的配置，
        /// 保证同一上下文类型只有一个配置节点
        /// </summary>
        /// <param name="options"></param>
        private void SetDbContextOptionses(OsharpOptions options)
        {
            IConfigurationSection section = _configuration.GetSection("Common:DbContexts");
            ...
            if (dict == null || dict.Count == 0)
            {
                ...
                {
                    DbContextTypeName = "Common.Entity.DefaultDbContext,Common.EntityFrameworkCore",
